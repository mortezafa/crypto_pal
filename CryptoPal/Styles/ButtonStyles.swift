...
struct PrimaryButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Self.Configuration) -> some View {
        configuration.label
            ...
            .padding()
            .background(configuration.isPressed || isDisabled ? Color.gray : Color.green)
            .cornerRadius(9)
...
struct SecondaryButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Self.Configuration) -> some View {
        configuration.label
            .frame(width: width, height: height) // Use optional binding in case width and height are not provided
            .padding()
            .background(configuration.isPressed ? Color.gray : Color.purple)
            .cornerRadius(9)
            .foregroundColor(.white)
            .scaleEffect(configuration.isPressed ? 0.95 : 1)
    }
...
struct PreviousButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                RoundedCorners(
                    color: !configuration.isPressed ? Color.black : Color.cyan.opacity(0.5),
                    tl: 0,
                    tr: 7,
                    bl: 0,
                    br: 7
...
struct NextButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                RoundedCorners(
                    color: isEnabled && !configuration.isPressed ? Color.pink : Color.pink.opacity(0.5),
                    tl: 7,
                    tr: 0,
                    bl: 7,
                    br: 0
